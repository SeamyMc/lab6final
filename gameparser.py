...
def remove_spaces(text):
    ...
    alpha_occurance = False

    for i in range (0,len(output)):
        if output[i] != " ":
            alpha_occurance = True
        elif alpha_occurance == False:
...
def normalise_input(user_input):
    ...
    sentence = remove_spaces(no_punct)
    word_list = sentence.split()
    normalised = filter_words(word_list,skip_words)    
