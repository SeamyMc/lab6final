    ...
    if len(items) == 0:
        ...
        for i in range(len(items)):
            if i == len(items) or i == 0:
                items_string = items_string + items[i]["name"]
            else:
                items_string = items_string + ", " + items[i]["name"]
    ...
    for item in current_room["items"]:
        if item_id == item["id"]:
            for key in inventory:
                total_mass = key["mass"] + total_mass
            total_mass = total_mass + item["mass"]
            if total_mass > 5:
                message = "This item is too heavy to take!"
            else:
                inventory.append(item)
                current_room["items"].remove(item)
                invalid_item = False
                message = "You took the " + item["id"]
    print(message)
...
def execute_drop(item_id):
    ...
    for item in inventory:
        if item_id == item["id"]:
            ...
            invalid_item = False
            message = "You dropped the " + item["id"]
    print(message)
